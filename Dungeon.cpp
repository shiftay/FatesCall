...
void Dungeon::Init(ScreenUtil* w, Player* p)
{
	cout << "|-->Dungeon::Init() Invoked" << endl;
	area = 0;
...
void Dungeon::Clean()
{
	cout << "|-->Dungeon::Clean() Invoked" << endl;
...
void Dungeon::HandleEvents(SDL_Event* event)
{
	//Event Handler
	switch (event->type)
	{
...
void Dungeon::Update(ScreenUtil* w)
{
...
void Dungeon::Render(ScreenUtil* w)
{
...
void Dungeon::ChangeRoom(SDL_Renderer* r)
{
	...
	case NORTH:
		...
		if (newRoom != EMPTY)
		{
			...
			case SHOP:
				...
				break;
			case BOSS:
				current_room = new Boss(r, HELLKNIGHT, area);
				isinBossR = true;
				break;
			case LOCKED:
...
// items
void Dungeon::MoveEnemies(SDL_Renderer* r)
{
	if (!current_room->ActiveUnits().empty())
	{
		for (int i = 0; i < current_room->ActiveUnits().size(); i++)
