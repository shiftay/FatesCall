...
#include "ThirdDungeon.h"

void ThirdDungeon::Init(ScreenUtil* w, Player* p)
{
	cout << "|-->ThirdDungeon::Init() Invoked" << endl;
	area = 2;
...
void ThirdDungeon::Clean()
{
	cout << "|-->ThirdDungeon::Clean() Invoked" << endl;
...
void ThirdDungeon::HandleEvents(SDL_Event* event)
{
	//Event Handler
	switch (event->type)
	{
...
void ThirdDungeon::Update(ScreenUtil* w)
{
	int count = 0;
...
void ThirdDungeon::Render(ScreenUtil* w)
{
...
void ThirdDungeon::ChangeRoom(SDL_Renderer* r)
{
	...
	case NORTH:
		...
		if (newRoom != EMPTY)
		{
			...
			case SHOP:
				...
				break;
			case BOSS:
				current_room = new Boss(r, FLYTRAP, area);
				isinBossR = true;
				break;
			case LOCKED:
...
void ThirdDungeon::CheckActiveItems()
{
	if (!ActiveItems.empty())
	{
		for (int i = 0; i < ActiveItems.size(); i++)
...
// items
void ThirdDungeon::MoveEnemies(SDL_Renderer* r)
{
	if (!current_room->ActiveUnits().empty())
	{
		for (int i = 0; i < current_room->ActiveUnits().size(); i++)
